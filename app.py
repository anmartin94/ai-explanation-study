...
def load_new_homeowner_graphic(header_placeholder):
    ...
    os.write(f"Participant ID: {user_id}, Image: {image_paths[st.session_state['current_image_index']]}, User choice: {st.session_state.get('feedback', 'N/A')}, Duration: {duration:.2f} seconds")
...
def load_new_homeowner(header_placeholder):
    ...
    os.write(f"Participant ID: {user_id}, Image: {image_paths[st.session_state['current_image_index']]}, User choice: {st.session_state.get('feedback', 'N/A')}, Duration: {duration:.2f} seconds, Query Count: {submit_count}")
        ...
        if 'chosen_option' in st.session_state:
            if st.session_state['chosen_option'] == 1:
                ...
                if submit_button and user_query:
                    ...
                    os.write(f"Participant ID: {st.session_state['user_id']}, Image: {image_paths[st.session_state['current_image_index']]}, User Query: {user_query}, System Response: {response}")
